...
from PACA_layers.MLP import MLP
from PACA_layers.ConditionalNAF_DSF import ConditionalNAF_DSF as CondNAF
from PACA_layers.ConditionalAffineFlow import ConditionalAffineFlow as CondAF
from PACA_layers.ConditionalTransformedDistribution import ConditionalTransformedDistribution as CondTDist
...
#         return loss
    ...
    def loss_function_train(self, outputs, label, r_bar,epoch):
        loss=self._compute_loss(*outputs,label, r_bar,epoch)
        return {"Loss":loss}

    def _compute_loss(self, output, emb, label, r_bar,epoch):
        adj = self.label_edge.data + torch.eye(self.label_edge.data.size(0),
                                                dtype=self.label_edge.data.dtype,
                                                device=self.label_edge.data.device)
        if epoch<=5:
            ...
            g = self.mask_matrix(output,label, r_bar)
            mask_loss_matrix=g*F.binary_cross_entropy_with_logits(output,label, reduction='none')
            sample_mask_loss= mask_loss_matrix.sum(dim=1).mean()

            loss = sample_mask_loss + 1e-3 * self.creation2(emb, adj)
        return loss
    ...
    def mask_matrix(self, preds, targets, r_bar):
        # 确保 targets 是张量
        if not isinstance(targets, torch.Tensor):
            targets = torch.tensor(targets, device=preds.device, dtype=torch.float)

        batch_size, q = targets.shape
        g = torch.ones_like(targets)  # 初始化掩码矩阵

        count_ones = targets.sum(dim=0)  # 每个类的正样本数量
        count_zeros = (targets == 0).sum(dim=0)  # 每个类的负样本数量
        ratio = (count_ones / (count_zeros + 1e-10)).clamp(0, 1)  # 当前类的正负样本比例
        rc = ratio.tolist()

        # 生成掩码概率矩阵
        mask_prob = torch.ones(q, device=preds.device)  # 初始化掩码概率
        for j in range(q):
            r_c = rc[j]
            if r_c > r_bar[j] and count_ones[j] > 0:
                mask_prob[j] = r_bar[j] / r_c
            elif r_c < r_bar[j] and count_zeros[j] > 0:
                mask_prob[j] = r_c / r_bar[j]

        # 使用广播生成掩码样本
        mask_samples = (torch.rand(batch_size, q, device=preds.device) < mask_prob).float()
        g = mask_samples  # 更新掩码矩阵
...
class DELA(nn.Module):
    ...
    def loss_function_train(self, preds: Tuple[Tensor, ...], targets: Tensor, r_bar,epoch) -> dict:
        Loss, Kl_loss, Cls_loss = self._compute_loss(*preds, targets, r_bar,epoch) 
    ...
    def _compute_loss(self, z: Tensor, n_logvar: Tensor, samples: Tensor,
                      preds: Tensor, targets: Tensor, r_bar,epoch) -> Tuple[Tensor, ...]:

        if epoch<=5:
            Cls_loss = F.multilabel_soft_margin_loss(preds, targets) * targets.size(1)
        else:
            g = self.mask_matrix(preds, targets, r_bar)
            preds_sigmoid = torch.sigmoid(preds)

            # 计算每个样本、每个标签的二元交叉熵损失
            loss_matrix = F.binary_cross_entropy_with_logits(preds, targets, reduction='none')

            # 使用掩码矩阵，将掩码为 0 的位置的损失置为 0
            masked_loss_matrix = loss_matrix * g

            # 对每行（每个样本）求和，得到每个样本的总损失
            sample_loss_sum = masked_loss_matrix.sum(dim=1)

            # 对所有样本的损失取平均
            Cls_loss = sample_loss_sum.mean()* targets.size(1)
...
#         # 计算每个样本的损失
#         individual_losses = F.binary_cross_entropy_with_logits(preds, targets, reduction='none')

#         # 应用掩码到损失
#         masked_losses = g * individual_losses

#         # 计算总损失
#         total_loss = masked_losses.sum() / (g.sum() + 1e-10)
...
class PACA(nn.Module):
    ...
    def loss_function_train(self, preds: Tuple[Tensor, ...], targets: Tensor, r_bar,epoch) -> dict:
        Loss, Recons_loss, Reg_loss, Cls_loss = self._compute_loss(*preds, targets, r_bar,epoch) 
...
#                       preds_y: Tensor, dists_x: Tensor, targets: Tensor, r_bar,epoch) -> Tuple[Tensor, ...]:
...
#         return Loss, Recons_loss, Reg_loss, Cls_loss
    ...
    def _compute_loss(self, input: Tensor, kl_div: Tensor, recons: Tensor,
        ...
        # Latent space regularization loss
            ...
            g = self.mask_matrix(preds_y, targets, r_bar)
            preds_sigmoid = torch.sigmoid(preds_y)

            # 计算每个样本、每个标签的二元交叉熵损失
            loss_matrix = -(targets * torch.log(preds_sigmoid + 1e-10) + (1 - targets) * torch.log(1 - preds_sigmoid + 1e-10))
            ...
            # 对所有样本的损失取平均
            mask_loss = sample_loss_sum.mean()* targets.size(1)
            Reg_loss = kl_div + mask_loss
